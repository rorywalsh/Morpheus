...
image bounds(13, 26, 25, 280), colour("white"), plant("primeLabelsPlant"),  identchannel("primeLabels_id"){
	label bounds(0, 0, 25, 15), text(""), fontcolour("black"), colour("white") widgetarray("primeLabels", 12)
}
 
;plant to hold array of widgets used to display retrograde row names. 
image bounds(522, 26, 20, 280), colour("white"), plant("retroLabelsPlant"), identchannel("retroLabels_id"){
	label bounds(0, 0, 20, 15), text(""), fontcolour("black"), colour("white") widgetarray("retroLabels", 12)
}
 
image bounds(548, 12, 118, 285), colour(200, 200, 200)         
image bounds(32, 304, 637, 95), colour(200, 200, 200)               
                                           
button bounds(560, 24, 95, 20), channel("resetRow"), text("Reset Matrix"), popuptext("Reset matrix back to blank state")
button bounds(560, 72, 95, 20), channel("rand"), text("Randon Row"), popuptext("Fill matrix with random values")
button bounds(560, 48, 95, 20), channel("resetSearch"), text("Reset Search"), popuptext("Clear search pattern")
button bounds(560, 272, 95, 20), channel("norm"), text("Norm"), popuptext("Normalises matrix - sets first note in row to 0 and scales all other notes accordingly")
...
groupbox bounds(552, 120, 106, 51) text("Spelling"), fontcolour("black"), colour("White"), 
combobox bounds(560, 144, 92, 24), channel("spellingCombo"), items("Sharps", "Flats", "10", "t"), value(3), popuptext("Change harmonic spelling of notes")
groupbox bounds(552, 168, 106, 55) text("Permutations"), fontcolour("black"), colour("White")
combobox bounds(560, 192, 92, 24), channel("permutationsCombo"), items("Select", "Original", "P1", "P2", "P3", "P4")
groupbox bounds(552, 216, 106, 52) text("Modulation"), fontcolour("black"), colour("White")
combobox bounds(560, 240, 92, 24), channel("modulationsCombo"), items("Select", "Original", "M5", "M7", "M11"), popuptext("Transposes each note by a set interval")
button bounds(560, 96, 95, 20), channel("classicRows"), text("Classic Rows", "Show Matrix"), value(0), popuptext("Shows a list of iconic rows used in various 12-tone compositions")

keyboard bounds(40, 312, 445, 79)

groupbox bounds(493, 312, 170, 81) text("Interval Class"), colour("black"), fontcolour("white"), colour("White")
label bounds(505, 336, 151, 17), text("[0 0 0 0 0 0]"), identchannel("intervalclass")
checkbox bounds(504, 360, 125, 20), channel("search"), fontcolour("white"), text("Search Matrix"), popuptext("Enable searched of matrix for a particular pattern")
...
opcode nameRowsAndColumns, 0, iiiii
	iP, iR, iI, iRI, iIndex xin
...
if chnget:i("search")==1 then
	...
	if iGo==0 then
		...
		if giNoteCount==1 then
								;p	i	ri	i	index
			nameRowsAndColumns 	0, 	0, 	0, 	0, 	1
		else 
			iRand =int(random(0, 11))
								;p		i		ri		i		index
			nameRowsAndColumns  iRand, 	iRand, 	iRand, 	iRand, 	giNoteCount
		endif
