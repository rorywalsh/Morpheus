<Cabbage>
form size(600, 600), caption(""), pluginID("plu1"), colour("white")
...
button bounds(360, 384, 65, 20), channel("resetRow"), text("Reset")
button bounds(360, 490, 65, 20), channel("rand"), text("Rand")
button bounds(360, 520, 65, 20), channel("norm"), text("Norm")
...
groupbox bounds(40, 490, 150, 71) text("Permutations"), fontcolour("black"), colour(200, 200, 200), plant("spelling"){
	button bounds(15, 25, 40, 20), channel("P1"), text("P1")
	button bounds(85, 25, 40, 20), channel("P2"), text("P2")
;	button bounds(15, 45, 40, 20), channel("P3"), text("P3")
}                             
...
button bounds(424, 384, 65, 20), channel("resetSearch"), text("Reset Search")
</Cabbage> 
<CsoundSynthesizer>
<CsOptions>
...
opcode getPX, i, j
	iDummyIn xin
		if chnget:i("P1")==1 then
		iPX = 0
		elseif chnget:i("P2")==1 then
		iPX = 1
...
;------------------------------------
opcode resetLabelColours, 0, ii
	iNumLabels, iResetText xin
	iCnt init 0
	iCntRows init 0
	...
	if iType==$ROW then 
		until iCntRow==12 do
			...
			until iCnt>=giSearchCount do
				until iCntCol==12 do
				iColCnt2 = iNumberOfSearches==iLEFT ? 23-iCntCol : iCntCol
				if giDisplayMatrix[iCntRow][iColCnt2]==giSearchArray[iCnt] then
				;found a match, now check subsequent indices
					until iCnt==giSearchCount do
					iCntCol3 = iNumberOfSearches==iLEFT ? 23-(iCntCol+iSubCnt) : iCntCol+iSubCnt
...
iMatches=0
enduntil
;if iFoundPattern==0 then
;resetArray(iProbTable, 12, -1)
;endif
...
instr SearchMatrixForRow
	...
	resetLabelColours(144,0); set all labels back to white
