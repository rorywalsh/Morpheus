...
combobox bounds(576, 192, 92, 24), channel("permutationsCombo"), items("Select", "Original", "P1", "P2", "P3", "P4")
...
opcode nameRowsAndColumns, 0, iiiii
	...
	SMessage sprintf "text(\"RI%d\")", iRI
	chnset SMessage, SChannel			
	SChannel sprintf "retroLabels_ident%d", iIndex
	SMessage sprintf "text(\"R%d\")", iR
	chnset SMessage, SChannel	
endop			
;================================================
...
if chnget:i("search")==1 then
	...
	if iGo==0 then
		...
		;	iRand =int(random(0, 11))
			...
			iPx = giNoteArray[0] - giNoteArray[giNoteCount]
			iPx wrap iPx,0,12
			
			iIx = 12 - iPx
			iIx wrap iIx,0,12
								;p		i		ri		i		index
			nameRowsAndColumns  iPx, 	iIx, 	iPx, 	iIx, 	giNoteCount
		endif
