...
image bounds(38, 504, 479, 20), plant("searchNotes"), visible(0), identchannel("searchNotesPlant"){
...
iNumLabels xin
	iCnt init 0
	until iCnt==iNumLabels do
			SChannel sprintf "note_ident%d", iCnt+1
			chnset "colour(0, 0, 0)", SChannel
			iCnt += 1 					
	enduntil
endop
...
instr SearchMatrixForRow
	...
	;resetLabelColours(144); set all labels back to black...
	...
	until iRow==12 do
		iScale = limit(log(iRowProbs[iRow]/(iMax+5) * (iSteepness-1)+1) / (log(iSteepness)), 0, 1)*100
		print iScale
		colourRow($ROW, iRow, 0, iRowProbs[iRow]*iScale, 0) 
		iRow+=1
	enduntil
