...
if chnget:i("search")==1 then
	...
	event_i "i", "fillDisplayMatrix", 0, 1
...
endin
;-------------------------------------------
; fill giDisplayMatrix with values from on-screen matrix
; giDisplay matrix contains all the values from giMatrix, 
; but in the same order as they appear on screen
instr fillDisplayMatrix
	iCntRows init 0
	iCntCols init 0

	until iCntRows == 12 do
		until iCntCols == 12 do
			giDisplayMatrix[iCntRows][iCntCols] = giMatrix[11-iCntRows][11-iCntCols] 
			iCntCols += 1
		od
		iCntCols = 0
		iCntRows += 1
		...
		until iCntRows == 12 do
			until iCntCols == 12 do
				if giSearchArray[iNumNotesToSearch] == giDisplayMatrix[iCntRows][iCntCols] then
					SChannel sprintf "note_ident%d", iCntRows*12+iCntCols+1
